...
**MVP1 Bước 1-3: PASS — kịch bản chứng minh giá trị cốt lõi (mục 15) đã chạy thật, không mock.** Control Plane (Postgres + API, `cp.valeron.tech`) và CLI wrapper `company-ai` (`login/init/claude/codex/status/checkpoint/end`) đã dựng xong và test bằng traffic thật: Thanh sửa code + commit thật qua Claude Code (seat riêng), publish handoff thật; Hoàng đăng nhập seat khác, mở lại đúng task, Claude Code tự đọc đúng 100% những gì Thanh đã làm — không có bước chuyển giao thủ công nào. Chi tiết đầy đủ, gồm 4 bug thật tìm thấy khi build (Windows spawn, quoting, readline, lỗ hổng auth không mã pilot) ở `MVP1-PROGRESS.md`. Còn lại: dashboard tối giản (Bước 4, không chặn đường), test `codex`, checkpoint theo git hook.
