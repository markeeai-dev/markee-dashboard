...
**MVP1 (Bước 0-4): PASS toàn bộ — kịch bản chứng minh giá trị cốt lõi (mục 15) đã chạy thật, không mock.** Control Plane (Postgres + API, `cp.valeron.tech`), CLI wrapper `company-ai` (`login/init/claude/codex/status/checkpoint/end`), và dashboard tối giản 5 màn (`ops.valeron.tech`) đã dựng xong và test bằng traffic thật: Thanh sửa code + commit thật qua Claude Code (seat riêng), publish handoff thật; Hoàng đăng nhập seat khác, mở lại đúng task, Claude Code tự đọc đúng 100% những gì Thanh đã làm — không có bước chuyển giao thủ công nào. Chi tiết đầy đủ, gồm các bug thật tìm thấy khi build (Windows spawn, quoting, readline, lỗ hổng auth không mã pilot, CORS) ở `MVP1-PROGRESS.md`. Còn lại (không chặn đường): test `codex` thật, checkpoint theo git hook, kiểm tra dashboard bằng trình duyệt thật.
