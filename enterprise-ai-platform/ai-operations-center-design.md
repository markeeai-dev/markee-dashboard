...
> **🔒 v17 — `9router` xác nhận là package thật (npm/Docker `decolua/9router`), đã cài/chạy/gọi API thật để verify — không còn suy đoán.** Xác nhận: port 20128, endpoint `/v1/messages` + `/v1/chat/completions`, hai tầng auth (admin session vs API key riêng — Adapter phải đổi sang API key này, không forward token Center AI, đã sửa lỗi này trong code spike), có API usage/telemetry sẵn. **Phát hiện quan trọng**: README chính thức xác nhận "Multi-account round-robin" và "Auto fallback sang FREE provider (Kiro AI, OpenCode Free)" là tính năng quảng cáo thật, không phải rủi ro giả định — container-per-seat + đúng 1 connection/instance, không combo, không free provider, là **bắt buộc tuyệt đối** (xem Q9, rủi ro Cao ở mục 17). Code spike (`spike/gateway-adapter/server.js`) đã sửa theo — test lại 7/7 vẫn PASS.
...
### Đã xác nhận thật (npm/Docker `9router`/`decolua/9router`, không còn suy đoán)

Đã cài thật, chạy thật container Docker (`decolua/9router:latest`) và gọi API thật:

- Default port **20128**, healthcheck `GET /api/health` → `{"ok":true}`, ảnh Docker persist qua volume `/app/data` + env `DATA_DIR` — khớp đúng model container-per-seat đã thiết kế.
- Endpoint proxy chính là `/v1/messages` (Anthropic-compatible) và `/v1/chat/completions` (OpenAI-compatible) — đúng dạng CLI wrapper/Claude Code sẽ gọi.
- **Hai tầng xác thực khác nhau, không trộn lẫn**: (1) admin session (đăng nhập password/OIDC vào dashboard) để cấu hình provider/key/policy — dùng cho các endpoint `/api/providers`, `/api/keys`, `/api/provider-nodes`, `/api/combos`, `/api/settings`; (2) API key riêng do chính 9Router phát hành cho từng connection, dùng để gọi `/v1/messages` — **Gateway Adapter phải cầm đúng loại (2), không phải token nghiệp vụ Center AI** (đã sửa lỗi này trong code spike — trước đó Adapter forward nhầm token Center AI lên thẳng 9Router, bị 9Router từ chối 401 "API key required").
- 9Router có sẵn API usage/telemetry khá đầy đủ (`/api/usage/stats`, `/api/usage/request-logs`, `/api/usage/[connectionId]`...) — tốt hơn dự đoán ban đầu, có thể tận dụng cho Request Span tầng MVP2 thay vì tự xây hết.
- Bootstrap password lần đầu (`hasPassword:false` lúc chưa cấu hình) **không thấy cách set qua API thuần** trong lần kiểm tra — có cơ chế khoá sau vài lần sai mật khẩu, nên **không đoán mò** — cần làm qua giao diện dashboard (mở cổng tạm/port-forward) lúc setup thật lần đầu cho mỗi instance.

> ⚠️ **Phát hiện quan trọng — xác nhận rủi ro đã cảnh báo là CÓ THẬT, không còn là giả thuyết**: README chính thức của 9Router quảng cáo thẳng các tính năng: *"Multi-account — Round-robin between accounts per provider"*, *"Auto fallback: Subscription → Cheap → Free, zero downtime"*, và hướng dẫn kết nối **"FREE provider (no signup)"** như *Kiro AI (free Claude unlimited)* hay *OpenCode Free (no auth)*. Đây là tính năng cốt lõi được quảng bá, không phải cấu hình sai hiếm gặp. Hệ quả bắt buộc:
> 1. **Container-per-seat không còn là "để an toàn hơn" — là bắt buộc tuyệt đối**, vì nếu dùng 1 instance chứa nhiều account, round-robin sẽ tự động kích hoạt đúng như quảng cáo.
> 2. **Tuyệt đối không cấu hình `combos`/fallback chain trộn nhiều account, và không kết nối bất kỳ "free provider" nào** (Kiro AI, OpenCode Free...) vào instance phục vụ công ty — request của nhân viên có thể bị âm thầm route qua một dịch vụ proxy không rõ nguồn gốc, rủi ro rò rỉ code/dữ liệu khách hàng ra ngoài. Đúng nghĩa `pool_type=FREE_OR_EXTERNAL` đã định nghĩa ở Q24.9: mặc định tắt, chỉ bật sau security review — với 9Router, khuyến nghị **không bao giờ bật cho instance công ty**, không chỉ "mặc định tắt".
> 3. Mỗi container chỉ nên cấu hình **đúng 1 provider connection**, không thêm combo/fallback nào khác — biến "round-robin" thành bất khả thi về mặt cấu hình, không chỉ dựa vào kỷ luật vận hành.
...
| Mức | Rủi ro | Mô tả |
|---|---|---|
| **Cao — ĐÃ XÁC NHẬN, không còn là giả thuyết** | **9Router tự round-robin/fallback qua connection của người khác, và tự động fallback sang "free provider" không rõ nguồn gốc (Q9, Q24.9)** | Kiểm tra README/API thật của package `9router` (npm + Docker `decolua/9router`) xác nhận: "Multi-account round-robin" và "Auto fallback Subscription → Cheap → Free" (kèm provider free như Kiro AI/OpenCode Free) là **tính năng quảng cáo chính thức**, không phải cấu hình sai hiếm gặp. Nếu Gateway Adapter không ép container-per-seat + đúng 1 provider connection/instance (không cấu hình `combos`), request của Thanh có thể lặng lẽ fallback qua seat của Hoàng hoặc qua 1 proxy free không rõ nguồn gốc — vi phạm thẳng nguyên tắc "1 seat = 1 người" (v2) và có nguy cơ rò rỉ code/dữ liệu khách hàng ra ngoài. Bắt buộc kiểm tra invariant này ở Bước 0 (Q24.10), và cấu hình mỗi instance chỉ 1 connection, không có combo/fallback nào. |
